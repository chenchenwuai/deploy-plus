...
// 备份远程文件
const moveRemoteFile = async (config, index) => {
  try {
    const { webDir } = config

    log(`(${index}) 备份远程文件 ${underline(webDir)} -> ${webDir}_backup`)

    await ssh.execCommand(`mv ${webDir} ${webDir}_backup`)
...
const createTaskList = (config, ignoreActions) => {
  ...
  isBackupRemoteFile && taskList.push(moveRemoteFile)
